...
<script type="text/javascript">
  $(function () {
    ...
    $("#tb_news_filter input").keyup(function(e) {
        var search = $(this).val();
        if(search != ''){
            $("#tb_news tbody").sortable('disable');
        }else{
            $("#tb_news tbody").sortable('enable');
        }
    });

    $("#tb_news tbody").sortable({
    ...
    $( "#tb_news tbody" ).disableSelection();

  });

    function del_news(id, images, member_id){
        $('#action').val('delete');
        $('#id').val(id);
        $('#images').val(images);
